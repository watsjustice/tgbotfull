...
export class SceneGenerator{
		...
		acc.on('text', async ctx => {
			...
			try{
				if (prevData.length > 1){
					let prevData = fs.readFileSync('userdata.json')
					prevData = JSON.parse(prevData)
					fs.writeFileSync('userdata.json', JSON.stringify([...prevData, data]));
				} else {
					let prevData = fs.readFileSync('userdata.json')
					prevData = JSON.parse(prevData)
					fs.writeFileSync('userdata.json', JSON.stringify([prevData, data]));
	...
	ReSendAction(){
		...
		messages.on('text', async ctx => {
			...
			let userInfoIds = fs.readFileSync('userdata.json')
			userInfoIds = JSON.parse(userInfoIds)
			for (let item in userInfoIds){
				let datnow = userInfoIds[item].date
				let dat = new Date()
				dat = String(dat.getFullYear()) + '-' + '0'+String(dat.getMonth()+1) + '-' + String(dat.getDate())
				if (new Date(datnow) - new Date(dat) < 24*60*60*1000*userInfoIds[item].duration){
					ctx.telegram.sendMessage(userInfoIds[item].id,
